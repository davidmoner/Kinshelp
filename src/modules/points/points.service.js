...
function transfer({ matchId, fromUserId, toUserId, amount }) {
...
function award({ userId, amount, reason = LEDGER_REASON.MATCH_COMPLETED, matchId = null, forceLedger = false }) {
...
function grant({ userId, amount, reason = LEDGER_REASON.ADMIN_GRANT, matchId = null }) {
    const now = new Date().toISOString();
    return repo.credit(userId, amount, matchId, reason, now);
}

function getLedger(userId, opts) {
    return { balance: repo.getBalance(userId), ledger: repo.ledger(userId, opts) };
}
...
function leaderboard(opts) {
    ...
    const out = lb.getLeaderboardResult(opts);
    setCachedLb(key, out);
    return out;
}

function leaderboardMe(userId, opts) {
    const rows = lb.getSortedUsers(opts);
